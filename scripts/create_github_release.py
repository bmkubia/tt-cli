#!/usr/bin/env python3
"""
Publish a tagged tt-cli release using the GitHub API.

This script uses the `gh` CLI tool to interact with the GitHub API. It creates a new commit that updates the version in `Cargo.toml`, creates a tag for that commit, and pushes the tag to the repository.
"""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish a tagged tt-cli release.")
    parser.add_argument(
        "-n",
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--promote-alpha",
        metavar="VERSION",
        help="Promote an existing alpha tag (e.g., 0.56.0-alpha.5) by using its merge-base with master as the base commit.",
    )
    ...
    parser.add_argument(
        ...
        help="Publish a specific version when the previous tag failed. Value should be semver, e.g., `0.43.0-alpha.9`.",
    )
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if promote_alpha:
            ...
            if args.dry_run:
                print(
                    f"Would publish version {version} using base commit {base_commit} derived from {TAG_PREFIX}{promote_alpha}."
                )
                return 0
        elif args.dry_run:
            return 0
...
def get_promote_alpha_base_commit(alpha_version: str) -> str:
    tag_name = f"{TAG_PREFIX}{alpha_version}"
    tag_commit_sha = get_tag_commit_sha(tag_name)
    return get_merge_base_with_master(tag_commit_sha)
...
def get_merge_base_with_master(commit_sha: str) -> str:
    response = run_gh_api(f"/repos/{REPO}/compare/master...{commit_sha}")
    try:
        return response["merge_base_commit"]["sha"]
    except KeyError as error:
        raise ReleaseError("Unable to determine merge base with master.") from error
...
def _strip_v(version: str | None) -> str | None:
    if not version:
        return None
